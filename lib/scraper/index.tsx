...
export async function scrapeAmazonProduct(url: string) {
  ...
  // ✅ FIX 3: Strip all tracking params — convert to clean /dp/ASIN url
  const asinMatch = url.match(/amazon\.([a-z.]+)\/(?:.*\/)?dp\/([A-Z0-9]{10})/);
  const cleanUrl = asinMatch
    ? `https://www.amazon.${asinMatch[1]}/dp/${asinMatch[2]}`
    : url;

  console.log("------------------------------------------------");
  console.log("Attempting scrape:", cleanUrl);
  ...
  try {
    ...
    // ✅ FIX 1: Removed session_id — it was mangling the username and causing 407
    // ✅ FIX 2: Added -country-in so BrightData routes through Indian IP for amazon.in
    const httpsAgent = new https.Agent({
      rejectUnauthorized: false,
    });
